...
export function renderGuideCorpus(pack: GuidePack): string {
  ...
  // 硬机制:特殊炮击编成判定(锁船分支判断的依据,严禁凭印象放宽)
  parts.push(`\n# 硬机制:特殊炮击(touch)编成判定(来源:KC3Kai 源码,以此名单为准)
- 大和型二舰式(旗舰=大和改二/大和改二重):2号位僚舰**仅限** 武蔵改二、Iowa改、Bismarck drei、Richelieu改、Richelieu Deux、Jean Bart改。※武蔵改(未改二)不在名单,不可触发!
- 大和型三舰式(旗舰=大和改二/重,2、3号位各一):可选僚舰=武蔵改二、長門改二、陸奥改二、伊勢改二、日向改二、扶桑改二、山城改二、Nelson改、Rodney改、Warspite改、Valiant改、金剛改二丙、比叡改二丙、榛名改二乙、榛名改二丙、霧島改二丙、South Dakota改、Washington改、Italia、Roma改、Colorado改、Maryland改、Jean Bart改、Richelieu改、Richelieu Deux。
- 武蔵二舰式:旗舰=武蔵改二(僚舰规则同源码)。
- 長門/陸奥一斉射:旗舰=長門改二 或 陸奥改二,2号位需战舰。
- Nelson Touch:旗舰=Nelson(改)/Rodney(改),第3、5位参与炮击。
- Colorado特殊攻击:旗舰=Colorado(改)/Maryland(改),第2、3位需战舰。
- 金剛型僚舰夜战突击:旗舰=金剛改二丙/比叡改二丙/榛名改二乙/榛名改二丙/霧島改二丙(仅这些形态)。
- 判定多号机"是否可用"要看**功能形态**(如 touch 兼容),不是物理存在;形态不符但等级接近改造门槛的,按"可用但需先改造"处理。`);
...
判定与分支规则(重要):
- **已到/接近改造等级但未改造的舰,不算"没有"**:按可用处理,level_note 标注「需先改造:当前LvX→需LvY,消耗XX(设计图/战报等)」;只有真缺失或练度差距过大才降级替代。
- 多号机可用性按**功能形态**判定(touch 兼容形态见硬机制名单):物理存在但形态不符时,先评估"改造后可用"的成本,再决定走哪个攻略分支。
- 攻略给出条件分支时(如武大二号机→E2 / 无则长陆 / 有大和2+鸭则E3大鸭),逐一核对 box 的功能可用性后**选攻略明确的优先分支**;备选方案(如长陆水打)只在优先分支确实不可行时才用,并在 reason 里说明为什么放弃优先分支。`;
